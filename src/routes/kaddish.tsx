...
export const Route = createFileRoute("/kaddish")({ component: Kaddish });
  ...
  return (
    <MobileFrame bg="navy" showNav={false}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center text-white">
        ...
        <p className="text-sm text-white/70 mt-3 max-w-xs leading-relaxed">{t("kaddish.subtitle")}</p>

        <div className="mt-8 w-full max-w-sm space-y-3">
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4 text-left">
            <label className="text-[10px] uppercase tracking-wider text-white/50">{t("kaddish.forWhom")}</label>
            <input className="w-full bg-transparent outline-none text-sm mt-1 placeholder:text-white/40" defaultValue="Avraham ben Yitzchak" />
          </div>
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4 text-left">
            <label className="text-[10px] uppercase tracking-wider text-white/50">{t("kaddish.personalNote")}</label>
            <input className="w-full bg-transparent outline-none text-sm mt-1 placeholder:text-white/40" placeholder={t("kaddish.notePh")} />
          </div>
        </div>
      ...
      <div className="px-6 pb-10 space-y-3">
        <Link to="/success" className="block w-full text-center gold-gradient text-gold-foreground font-semibold py-4 rounded-2xl shadow-glow-gold">
          {t("kaddish.send")}
        </Link>
