    ...
    <MobileFrame>
      ...
      <div className="mx-6 rounded-3xl overflow-hidden navy-gradient text-white p-6 shadow-lift relative">
        ...
        <div className="relative flex items-start justify-between gap-4">
          ...
          <button onClick={() => setOn(!on)} className={`h-7 w-12 rounded-full relative transition-colors ${on ? "bg-gold" : "bg-white/20"}`}>
      ...
      <div className="mx-6 mt-4 rounded-2xl bg-surface border border-border p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground">{t("backup.alertRadius")}</div>
            <div className="font-display text-lg">{radiusLabel}</div>
          </div>
          <MapPin className="h-5 w-5 text-muted-foreground" />
        </div>
        <input type="range" min={200} max={3000} step={100} value={radius} onChange={(e) => setRadius(Number(e.target.value))} className="w-full accent-[oklch(0.82_0.14_80)]" />
      ...
      <div className="mx-6 mt-4 rounded-2xl bg-surface border border-border divide-y divide-border">
        <Rule icon={Bell} title={t("backup.ruleCriticalTitle")} desc={t("backup.ruleCriticalDesc")} />
        <Rule icon={Clock} title={t("backup.ruleQuietTitle")} desc={t("backup.ruleQuietDesc")} />
        <Rule icon={Check} title={t("backup.ruleConfirmTitle")} desc={t("backup.ruleConfirmDesc")} />
      </div>

      <div className="px-6 mt-5">
        <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">{t("backup.lastWeek")}</div>
      ...
      <div className="px-6 pt-5 pb-4">
        <Link to="/home" className="w-full gold-gradient text-gold-foreground font-semibold py-4 rounded-2xl shadow-glow-gold flex items-center justify-center gap-2">
...
function Rule({ icon: Icon, title, desc }: any) {
  return (
    <div className="p-4 flex items-start gap-3">
      <div className="h-9 w-9 rounded-xl bg-muted flex items-center justify-center shrink-0"><Icon className="h-4 w-4" /></div>
